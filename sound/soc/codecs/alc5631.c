 ...
 * 
 * Modified 2011 by <saadia@laptop.org> One Laptop Per Child
 *
 * Based on WM8753.c
 *
...
static int dc_mode_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.enumerated.item[0] = gpio_get_value(GPIO_DC_MODE);
	return 0;
}

static int dc_mode_put(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	int setting = ucontrol->value.enumerated.item[0];
	struct snd_soc_codec *codec = kcontrol->private_data;

	if (setting != 0) {
		snd_soc_update_bits(codec, RT5631_MIC_CTRL_2, 0xff00, 0);
		snd_soc_update_bits(codec, RT5631_GEN_PUR_CTRL_REG, 0x0400, 0);
	}
	else {
		snd_soc_write(codec, RT5631_MIC_CTRL_2, 0x5500);
		snd_soc_update_bits(codec, RT5631_GEN_PUR_CTRL_REG,
				    0x0400, 0x0400);
	}
	gpio_set_value(GPIO_DC_MODE, setting);
	return 0;
}
...
static const struct snd_kcontrol_new rt5631_snd_controls[] = {
	...
	SOC_SINGLE_BOOL_EXT("DC Mode Enable", 0, dc_mode_get, dc_mode_put),
	/* MONO IN */
	...
	SOC_DOUBLE_TLV("AXI Capture Volume", RT5631_AUX_IN_VOL,
			...
			RT5631_VOL_MASK, 1, in_vol_tlv),
	/* DAC */
	SOC_DOUBLE_TLV("Master Playback Volume", RT5631_STEREO_DAC_VOL_2,
			RT5631_L_VOL_SHIFT, RT5631_R_VOL_SHIFT,
			RT5631_DAC_VOL_MASK, 1, dac_vol_tlv),
	SOC_DOUBLE("Master Playback Switch", RT5631_STEREO_DAC_VOL_1,
...
#if 0 // This was remaining powered on when not needed
	/* ADC Data Source */
...
#if 0 // Removed routes to Left ADC Select
...
#if 0 // Removed routes to Right ADC Select
		...
		struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	rt5631->bclk_rate = rt5631->rx_rate * timesofbclk;
	coeff = get_coeff(rt5631->sysclk, rt5631->rx_rate, timesofbclk);
	if (coeff < 0) {
...
static int alc5631_mute(struct snd_soc_dai *dai, int mute)
{
	struct snd_soc_codec *codec = dai->codec;
	u32 tmp = snd_soc_read(codec, RT5631_PWR_MANAG_ADD1);
	bool is_capture =  tmp & (RT5631_PWR_ADC_L_CLK | RT5631_PWR_ADC_R_CLK);
	...
	if (mute) {
		...
		snd_soc_update_bits(codec, RT5631_ADC_CTRL_1, 0x8080, 0);
		if (is_capture)
			snd_soc_update_bits(codec, RT5631_PWR_MANAG_ADD2,
...
int alc5631_spkr_mute(struct snd_soc_codec *codec, int mute)
{
	struct rt5631_priv *rt5631 = snd_soc_codec_get_drvdata(codec);

	pr_info("SETTING SPEAKER MUTE TO %d\n", mute);
...
static int rt5631_probe(struct snd_soc_codec *codec)
{
	...
	snd_soc_dapm_new_controls(dapm, rt5631_dapm_widgets,
 	...
 	snd_soc_dapm_add_routes(dapm, rt5631_dapm_routes,
