				     ...
				     unsigned long flags)
{
	...
	while (size && *cur_order) {
		...
		/* TODO:
		 * remove __GFP_COMP after vithar UMP chanages vm_insert_mixed()
		 * against physical memory addresses retrieved from ION buffer.
		 * __GFP_COMP will split the large contiuous compound pages into
		 * 4KB pages and make slower this function.
		 */
			...
			struct ion_buffer *buffer, struct vm_area_struct *vma)
{
	...
	/* NOTE:
	 * this function does not support COW mapping. mmap() with MAP_PRIVATE
	 * and file descriptors generated by ION will return fail.
	 */
	vma->vm_flags |= VM_DONTEXPAND;

	for (sgl = sgt->sgl; (sgl != NULL) && (size > 0); sgl = sg_next(sgl)) {
		unsigned long sg_size = sg_dma_len(sgl);
		int ret;
		...
		sg_size -= off;
		if (unlikely(size < sg_size))
			sg_size = size;

		ret = remap_pfn_range(vma, start,
			page_to_pfn(sg_page(sgl)) + (off >> PAGE_SHIFT),
			sg_size, vma->vm_page_prot);
		if (ret)
			return ret;
