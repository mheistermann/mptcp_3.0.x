...
static const struct s5k4ecgx_resolution s5k4ecgx_resolutions[] = {
	...
	/*TODO: Prevent to max size preview because it is selected
	 * when user want to capture mode. */
...
static int s5k4ecgx_power(int flag)
{
   ...
   //xiebin.wang@20110530
	...
	{
		//powerdown
		if (gpio_request(GPIO_CAM_MEGA_nRST, "GPX1_2"/*"GPJ1"*/) < 0)
		...
		if (gpio_request(GPIO_CAM_MEGA_EN, "GPF1_0"/*"GPJ0"*/) < 0)
