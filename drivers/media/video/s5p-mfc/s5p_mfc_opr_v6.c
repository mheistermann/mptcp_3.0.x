...
int s5p_mfc_alloc_dev_context_buffer(struct s5p_mfc_dev *dev)
{
	...
	alloc_ctx = dev->alloc_ctx_drm;
#endif
	dev->ctx_buf.alloc = s5p_mfc_mem_alloc_priv(alloc_ctx, buf_size->dev_ctx);
	...
	s5p_mfc_mem_clean_priv(dev->ctx_buf.alloc, dev->ctx_buf.virt, 0,
			buf_size->dev_ctx);
...
static inline int s5p_mfc_run_init_dec_buffers(struct s5p_mfc_ctx *ctx)
{
	...
	temp_vb = list_entry(ctx->src_queue.next, struct s5p_mfc_buf, list);
	mfc_debug(2, "Header size: %d\n", temp_vb->vb.v4l2_planes[0].bytesused);
	s5p_mfc_set_dec_stream_buffer(ctx, s5p_mfc_mem_plane_addr(&temp_vb->vb, 0),
				0, temp_vb->vb.v4l2_planes[0].bytesused);
	spin_unlock_irqrestore(&dev->irqlock, flags);
#endif
	dev->curr_ctx = ctx->num;
