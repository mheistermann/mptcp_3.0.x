...
int fimc_is_v4l2_isp_scene_mode(struct fimc_is_dev *dev, int mode)
{
	...
	case SCENE_MODE_BACK_LIGHT:
		...
		break;
#endif
	/* FIXME add define SCENE_MODE_BACK_LIGHT (FLASH mode) */
		...
		if (!is_af_use(dev)) {
			...
			case IS_FOCUS_MODE_AUTO:
				...
				IS_ISP_SET_PARAM_AA_SCENE(dev,
					ISP_AF_SCENE_NORMAL);
				IS_ISP_SET_PARAM_AA_TOUCH(dev,
					ISP_AF_TOUCH_DISABLE);
				IS_ISP_SET_PARAM_AA_FACE(dev,
					ISP_AF_FACE_DISABLE);
				IS_ISP_SET_PARAM_AA_RESPONSE(dev,
					ISP_AF_RESPONSE_PREVIEW);
...
int fimc_is_v4l2_frame_rate(struct fimc_is_dev *dev, int value)
{
	...
	case 0: /* AUTO Mode */
		...
		IS_ISP_SET_PARAM_DMA_INPUT1_CMD(dev, DMA_INPUT_COMMAND_ENABLE);
		IS_ISP_SET_PARAM_DMA_INPUT1_FRAMETIME_MIN(dev, 0);
		IS_ISP_SET_PARAM_DMA_INPUT1_FRAMETIME_MAX(dev, 66666);
...
#ifdef FPS_ENABLE
		if (test_bit(FIMC_IS_STATE_HW_STREAM_ON, &dev->pipe_state) &&
			test_bit(FIMC_IS_STATE_SCALERP_STREAM_ON, &dev->pipe_state)) {
